...
"$GHIDRA_INSTALL_DIR/support/analyzeHeadless" "$PROJECT_DIR" "$PROJECT_NAME" \
  ...
  -scriptPath "$GHIDRA_AETHER_DIR/headless" \
...
The MCP server exposes one backend-independent Ghidra surface. It queries Programs already open in Ghidra during an interactive session, or creates project workspaces and imports new binaries into managed headless sessions when no interactive session is available. Use `create_project` for a new workspace or `open_project` to register an existing `.gpr`/`.rep` project without overwriting it; `close_project` releases that MCP project handle and stops its owned headless sessions. `import_binary` selects the available backend automatically and returns an import job; when no interactive bridge or project handle is available, it creates a unique temporary project under `AETHER_MCP_PROJECT_DIR` (default `/tmp/aether-ghidra-projects`). Poll `get_import_job` for the resulting opaque `analysis_id`. Each Program tool accepts that `analysis_id`; the broker resolves the underlying session and Program without exposing backend selection to agent workflows. Managed headless sessions remain alive until explicitly closed with `close_analysis_session`, their project is closed, or the MCP server shuts down. The startup timeout only bounds readiness checks and does not expire a ready analysis session. Agents do not provide a headless/interactive mode. It excludes chat, conversation, memory, and planning tools, while exposing program reads and writes.
...
Program operations use opaque broker `analysis_id` handles. The broker invalidates a handle when its backing session or Program closes and reports backend lifecycle failures with bounded headless diagnostics. Function-targeting MCP tools accept an exact structured `address`; returned function metadata also uses `address` as its sole identity field. Writes are serialized per Program and run inside Ghidra transactions. Function, variable, definition, and comment writes can be issued individually or as an atomic `apply_annotation_batch`; structure and class writes apply immediately. Class metadata is stored with the Program and references native Ghidra structures. RTTI is consumed from Ghidra's existing analysis output rather than exposed as a separate AETHER tool.
