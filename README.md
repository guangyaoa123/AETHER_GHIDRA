...
The configuration dialog also controls grouped chatbot tools. Program inspection, analysis context, planning, memory, and conversation tools are enabled by default. Program mutation tools such as function renaming and comment updates are disabled by default and must be enabled explicitly.
...
export AETHER_HEADLESS_DURATION_SEC=300
...
- `get_function_pseudocode` (commits current locals and parameters, then returns address-aware pseudocode; unmapped lines remain unprefixed)
- `get_data_at_address`
- `get_xrefs_to`
- `rename_function`
- `rename_variable`
...
All requests require an explicit `program_id`. Program IDs are valid only for the current Ghidra session and are removed when a Program closes. Writes are serialized per Program and run inside Ghidra transactions.
